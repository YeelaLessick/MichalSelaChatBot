...
resource "azuread_application_registration" "bot_app" {
...
resource "azuread_service_principal" "bot_sp" {
  client_id = azuread_application_registration.bot_app.id
}

# AAD Application Password (Client Secret)
resource "azuread_application_password" "bot_password" {
  application_id = azuread_application_registration.bot_app.object_id
...
resource "azurerm_bot_channels_registration" "michalselabot" {
  ...
  microsoft_app_id = azuread_application_registration.bot_app.id
}
